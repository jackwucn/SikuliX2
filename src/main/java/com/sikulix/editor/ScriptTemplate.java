...
public abstract class ScriptTemplate {
  ...
  static String convertScript(Script script, List<List<ScriptCell>> scriptData, File scriptFolder) {
    return convertScript(script, Runner.ScriptType.JAVASCRIPT, scriptData, scriptFolder);
  }

  static String convertScript(Script script, Runner.ScriptType type, List<List<ScriptCell>> scriptData, File scriptFolder) {
    String linePrefix = "//*** (%d) autogenerated --- do not edit\n%s";
    String linePostfix = "\n//*** end autogenerated\n";
    ...
    for (List<ScriptCell> line : scriptData) {
      ...
      if (createMethods.contains("create" + command)) {
        ...
        try {
          ...
          snippet += String.format(linePrefix, lineNumber - 1, snippetLine);
          snippet += linePostfix;
        } catch (Exception e) {
  ...
  public static String createfind(ArrayList<ScriptCell> line, Integer lineNumber) {
    ...
    String template = " %s = find(\"%s\", %s); print(%s);";
    snippet += String.format(template, result, what, where, result);
    return snippet;
  }

  public static String createfindAll(ArrayList<ScriptCell> line, Integer lineNumber) {
    String snippet = createfind(line, lineNumber);
    return snippet.replace("= find(", "= findAll(");
  }
  ...
  public static String createwait(ArrayList<ScriptCell> line, Integer lineNumber) {
    ...
    String template = " %s = wait(\"%s\", %s, %d); print(%s);";
  ...
  public static String createvanish(ArrayList<ScriptCell> line, Integer lineNumber) {
    String snippet = createwait(line, lineNumber);
    return snippet.replace("= wait(", "= waitVanish(");
  }
