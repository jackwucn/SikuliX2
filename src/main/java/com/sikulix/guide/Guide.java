...
public class Guide extends Overlay implements EventObserver {

  private static SXLog log = SX.getSXLog("SX.GUIDE");
...
//<editor-fold defaultstate="collapsed" desc="TODO not used: play steps">
  ...
  public Visual image(Object img) {
    ...
    if (gc != null) {
      ...
      log.error("image: invalid argument %s", img);
    }
    return gc;
  }
  ...
  public Visual arrow(Object from, Object to) {
    ...
    if (gc != null) {
      ...
      log.error("arrow: invalid argument %s - %s", from, to);
    }
    return gc;
  }
